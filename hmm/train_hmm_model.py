...
model_trong.fit(X=np.vstack(data_trong), lengths=[x.shape[0] for x in data_trong])

with open("model/model_trong.pickle", "wb") as file: pkl.dump(model_trong, file)

# with open("model/model_xuong.pickle", "wb") as file: pkl.dump(model_xuong, file)